...
//  var url = "ajax/getFromRfid.php?rfid="+rfid;
  var url = "ajax/getFromRfid.php?rfid="+rfid;
//  alert(url);
  $.getJSON( url, function( data ) {
    $("#info-title").html(data.lokid);
    $("#rfid-info").html(data.id);
    $("#name-info").html(data.lokname);
    $("#info-info").html(data.lokinfo);
    $("#comment-info").html(data.comment);
   var interval = setInterval(function(){
        $.mobile.loading('hide');
        clearInterval(interval);
    },1);
